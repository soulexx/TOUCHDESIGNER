...
TICK_CODE = textwrap.dedent(
    """\
    from project1.palette_logic import watchdog, pump, state
...
def _ensure_text_dat(parent, name, content):
    dat = parent.op(name)
    if not dat:
...
def run_fix():
    ...
    if not osc_in:
        ...
        if "osc_in_callbacks" not in str(callbacks):
            _warn(
                "OSC In callbacks Feld verweist nicht auf src/project1/io/osc_in_callbacks.py"
            )
